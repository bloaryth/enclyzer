#!/bin/bash
## Usage: sudo bash install-linux-sgx-2.14.sh
...
echo "[LINUX-SGX] Downloading linux-sgx-2.14..."
git clone https://github.com/intel/linux-sgx.git linux-sgx --branch sgx_2.14 --single-branch --depth 1
...
echo "[LINUX-SGX] Modifying linux-sgx-2.14..."
...
echo "[LINUX-SGX] Building linux-sgx-2.14 SDK..."
make sdk -j `nproc`
...
echo "[LINUX-SGX] Installing linux-sgx-2.14 SDK..."
...
echo "[LINUX-SGX] Building linux-sgx-2.14 PSW..."
make psw -j `nproc`
make psw_install_pkg

# Install the Intel(R) SGX PSW
echo "[LINUX-SGX] Installing linux-sgx-2.14 PSW..."
cd linux/installer/bin/
