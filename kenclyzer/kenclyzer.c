#include "kenclyzer/kenclyzer.h"
#include "kenclyzer/kenclyzer_ioctl.h"
...
long kenclyzer_get_pt_mapping(struct file *filep, unsigned int cmd, unsigned long arg)
{
...
long kenclyzer_rdmsr(struct file *filep, unsigned int cmd, unsigned long arg)
{
    msr_t *msr = (msr_t *)arg;
...
long kenclyzer_wrmsr(struct file *filep, unsigned int cmd, unsigned long arg)
{
    msr_t *msr = (msr_t *)arg;
...
long kenclyzer_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
    char data[256];
...
static const struct file_operations pmc_fops = {
    .owner = THIS_MODULE,
    .compat_ioctl = kenclyzer_ioctl,
    .unlocked_ioctl = kenclyzer_ioctl,
    .open = pmc_open,
    .release = pmc_release};
