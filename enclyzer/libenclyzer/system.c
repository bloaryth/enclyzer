...
#include "enclyzer/libenclyzer/system_t.h"
...
#include "enclyzer/libenclyzer/system_u.h"
...
int fd_enclyzer = FD_UNINITIALIZED;
...
void open_system_file(void)
{
    ...
    if (fd_enclyzer == FD_UNINITIALIZED)
        ASSERT((fd_enclyzer = open("/dev/kenclyzer", O_RDWR)) >= 0); /** /dev/enclyzer is opened. */
}
